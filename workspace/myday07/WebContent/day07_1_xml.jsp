...
<%@ page language="java" contentType="text/html; charset=UTF-8"
  ...
  <%
  //xml 데이터를 받아올 주소
  String url = "http://localhost:18888/myday07/books.xml";
  //xml 파싱을 위한 객체
  ...
  for(int i=0;i<book.getLength();i++){
	  Node n = book.item(i);	//<name></name> 안에 있는 값들이 node, element라고 불러.
	  Element e = (Element)n;
	  Map pub = new HashMap();
	...
	NodeList prices = e.getElementsByTagName("price");
	  ...
	  out.println(names.item(0).getNodeName());
	  out.println(names.item(0).getChildNodes().item(0).getNodeValue());
	  out.print("<br/>");
	  
	  out.println(prices.item(0).getNodeName());
	  out.println(prices.item(0).getChildNodes().item(0).getNodeValue());
	  out.print("<br/>");
