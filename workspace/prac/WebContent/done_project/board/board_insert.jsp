...
<body>
	...
	<!-- 실습하기. 로그인 되어있지 않은데 Auth로 접근할 경우 list로 보내버리세요 -->
	<c:if test = "${param.type eq 'Auth' and empty loginId }">
		<c:redirect url = "board/board_list.jsp"></c:redirect>
	</c:if>
	
	<!-- 로그인 여부에 따른 변수 세팅 -->
	<!-- 로그인 했으면 로그인 한 id, 로그인 안했으면 free로 세팅 -->
	<c:set var = 'createId' value = "${loginId }"></c:set>
